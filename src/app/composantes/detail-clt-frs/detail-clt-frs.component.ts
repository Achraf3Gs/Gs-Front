    ...
    this.cltfrsService.deleteClient(this.clientFournisseur.id).subscribe({
      next: (res: any) => {
       ...
       // this.suppressionReuslt.emit('success');
        this.suppressionReuslt.emit({ result: 'success', clients: this.listeCients });
      ...
      error: (error: any) => {
        ...
        if (error instanceof HttpErrorResponse) {
          if (error.status === 400 && error.error) {
            try {
              ...
              const errorMessage = errorResponse.message || ['An error occurred while deleting the client.'];
              ...
              const errorMessage = ['An unknown error occurred while deleting the client.'];
              console.error(errorMessage);
              this.errorMessage = errorMessage;
            }
          } else {
       ...
       console.log('Invalid SelectedArToDelete value:', this.clientFournisseur.id);
      }
}
  ...
  if (this.clientFournisseur.id) {
    ...
    this.cltfrsService.deleteFounisseur(this.clientFournisseur.id).subscribe({
      next: (res: any) => {
        console.log('Delete successful:', res);
       // this.suppressionReuslt.emit('success');
        this.suppressionReuslt.emit({ result: 'success', fournisseurs: this.listeFournisseurs });
        ...
        if (error instanceof HttpErrorResponse) {
          if (error.status === 400 && error.error) {
            try {
              const errorResponse = JSON.parse(error.error);
              console.log('Error Response:', errorResponse);
              const errorMessage = errorResponse.message || ['An error occurred while deleting the fournisseur.'];
              console.error(errorMessage);
              this.errorMessage = [errorMessage];
              this.suppressionReuslt.emit(this.errorMessage);
            } catch (e) {
            ...
            const errorMessage = ['An unknown error occurred while deleting the fournisseur.'];
            console.error(errorMessage);
            this.errorMessage = errorMessage;
          }
        } else {
